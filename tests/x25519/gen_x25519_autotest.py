...
def crc32(buf):
    ...
    crc ^= 0xFFFFFFFF
    for byte in buf:
        crc = crc32c_tab[(crc ^ byte) & 0xFF] ^ (crc >> 8)
    return crc ^ 0xFFFFFFFF

def crc32_of_list(number):
    bnumber = [number // 2**e % 256 for e in range(0, 249, 8)]
    checksum = crc32(bnumber)
    return checksum

def finv(x):
    p = x
    for r in range(253, -1, -1):
        p = p*p % (2**255 - 19)
        if r != 2 and r != 4:
            p = p*x % (2**255 - 19)
    return p
...
def ladder(point, k):
    ...
    for i in binary:
        ...
        e = a+c
        a = 2**255 - 19 + (a-c)  # substractions are done like this in x25519.asm
        c = b+d
        b = 2**255 - 19 + (b-d)
        d = (e*e)       % (2**255 - 19)
        f = (a*a)       % (2**255 - 19)
...
def generate_autotest(filename):
    json_dict = {
        "transfer_files": [
            "bin/X25519.8xp"
        ],
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        generate_autotest(sys.argv[1])
    else:
        print("Err: Missing autotest filename")
        exit(-1)
